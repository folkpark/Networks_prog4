...
if __name__ == '__main__':
    ...
    router_a_table = [['H1', 0, 1], ['H2', 1, 1], ['RB', 2, 1], ['RC', 3, 2]]  # [neighbor, interface, cost]
    router_a = network.Router(name='RA',
                              cost_D=router_a_table,
                              max_queue_size=router_queue_size)
    object_L.append(router_a)

    # cost_D = {'H2': {1: 3}, 'RA': {0: 1}} # {neighbor: {interface: cost}}
    router_b_table = [['RA', 0, 1], ['RD', 1, 5]]  # [neighbor, interface, cost]
    router_b = network.Router(name='RB',
                              cost_D=router_b_table,
                              max_queue_size=router_queue_size)
    object_L.append(router_b)

    router_c_table = [['RA', 0, 2], ['RD', 1, 2]]  # [neighbor, interface, cost]
    router_c = network.Router(name='RC',
                              cost_D=router_c_table,
                              max_queue_size=router_queue_size)
    object_L.append(router_c)

    router_d_table = [['RB', 0, 5], ['RC', 1, 2], ['H3', 2, 1]]  # [neighbor, interface, cost]
    router_d = network.Router(name='RD',
                              cost_D=router_d_table,
                              max_queue_size=router_queue_size)
    ...
    # (node_1, node_1_intf, node_2, node_2_intf)
